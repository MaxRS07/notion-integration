...
export const Dashboard: React.FC<DashboardProps> = ({ settings }) => {
  ...
  const list_courses = async () => {
    setInfoMsg("");
    try {
      let courses = await invoke("get_courses");
      let courseIds = (courses as Course[]).map(o => o.id);
      for (const cid of courseIds) {
        let assignments = await invoke("get_assignments", { courseId: cid.toString() });
        let names = (assignments as Assignment[]).map(a => a.name);
        setInfoMsg(prev => prev + ", " + names.join(", "));
      }
    } catch (err) {
      setInfoMsg(String(err));
    }
  ...
  return (
    <div className="dashboard">
      <div className="dashboard-content">
        <button onClick={list_courses}>Hello</button>
        <p>{infoMsg}</p>
        {!hasTokens ? (
          <div className="welcome-state">
            ...
            <h2 className="welcome-title">Welcome to Notion Integrations</h2>
